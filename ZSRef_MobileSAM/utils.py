...
For python-based LazyConfig, use "path.key=value".
    ...
    parser.add_argument('--dataset', default='refcoco', help='refcoco, refcoco+, refcocog or golfball') # 데이터셋 변경 ●
    ...
    # 데이터셋 경로 ●
    # --------------------------------------------------------------
    parser.add_argument('--parent-dataset-path',
                        default='/media/hi/SK Gold P31/Capstone/GolfBall/Crawling_cp', type=str,
                        help='불러올 부모 데이터셋 경로')  # '/media/hi/SK Gold P31/Capstone/GolfBall/Golfball_Near_Remove_Similar_FixLabel_Remove_BboxInBbox_Remove_ErrorBboxRatio/images/train'
    parser.add_argument('--child-folder-name',
                        default='golf ball in sand', type=str,
                        help='불러올 자식 데이터셋 폴더명')
    ...
    parser.add_argument('--before-file-extension', default='.jpg', type=str,
                        help='parent_dataset_path/child_dataset_path 안에 들어있는 이미지 파일 확장자')
    parser.add_argument('--after-file-extension', default='.txt', type=str,
                        help='auto label한 bbox 결과를 저장할 텍스트 파일 확장자')
    ...
    # 배경과 같이 큰 물체 : 세부적인 물체의 영향을 줄이기 위해 크게 설정 해야하는 줄 알았는데, 작은 물체 탐지할 때 처럼 작게 설정하는 게 더 성능 좋음 -> 전체 그림에서 mask를 씌운 것이 Global Visual이기 때문에, 나의 가정과 상관 없음 # 0.15 ~ 0.5
    # 골프공과 같이 작은 물체 : 세부적인 물체의 영향을 키우기 위해 작게 설정 but 주변 배경에 대한 정보를 받기 위해 적당히 크게 설정 # 0.15 ~ 0.5
    parser.add_argument('--r', default='0.5', type=float, help='Text Embedding 가중치 설정')
    # 꾸미는 어구가 많은 긴 문장의 경우 : 전체 문장에 대한 정보가 잘 반영되도록 크게 설정
    # 핵심 단어 위주로만 구성된 짧은 문장인 경우 : 핵심 단어의 영향을 키우기 위해 작게 설정 but 전체 문장에 대한 정보도 받기 위해 적당히 크게 설정 # 0.5

    # --------------------------------------------------------------
    # Mask & Bbox PostProcessing Setting ●
    # --------------------------------------------------------------
    parser.add_argument('--min-pred-mask-area', default='1200', type=int, help='Mask Generator에서 예측한 mask에서 남길 최소 mask 크기')
    # 배경과 같이 큰 물체 : 세부적인 물체의 영향을 줄이기 위해 크게 설정 # 10000
    # 골프공과 같이 작은 물체 : 노이즈의 영향이 없도록 최대한 크게 but 물체가 검출되도록 작게 설정 # 1250
    parser.add_argument('--visual-local-offset', default='100', type=int, help='Mask Generator에서 추출된 mask를 CLIP에서 사용하기 위한 offset')
    # 배경과 같이 큰 물체 : 세부적인 물체의 영향을 줄이기 위해 offset 작게 설정 # 0
    # 골프공과 같이 작은 물체 : 실제 CLIP은 크롤링 데이터로 학습한 것이므로, 물체의 경계면보다 더 큰 배경에 물체가 놓여있었을 것임. 그러므로, 주변 배경이 함께 포함되도록 적당한 offset 설정 # 50
