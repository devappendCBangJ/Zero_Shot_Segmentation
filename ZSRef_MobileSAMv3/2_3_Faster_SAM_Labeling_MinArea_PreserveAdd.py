...
class ClickableImageLabel(QLabel):
    ...
    def __init__(self, loaded_masked_image, loaded_class_nums, loaded_contours, loaded_scaled_contours, loaded_bboxes, loaded_scaled_bboxes, scaled_pixmap, real_image_size, abs_image_path, abs_seg_path, abs_bbox_path):
        ...
        self.loaded_bboxes = loaded_bboxes
        self.loaded_scaled_bboxes = loaded_scaled_bboxes
...
class ImageWindow(QWidget):
            ...
            for j in range(4):
                ...
                if idx < len(self.image_filenames):
                    ...
                    if not os.path.exists(abs_seg_path):
                        with open(abs_seg_path, 'w') as loaded_seg_txt:
                            pass
                    if not os.path.exists(abs_bbox_path):
                        with open(abs_bbox_path, 'w') as loaded_bbox_txt:
                            pass

                    # 4]] Label 기존 정보 불러오기 + 이미지에 그리기
                    loaded_class_nums = []
                    loaded_contours = []
                    loaded_scaled_contours = []
                    loaded_bboxes = []
                    loaded_scaled_bboxes = []
                    with open(abs_seg_path, 'r') as loaded_seg_txt:
                        if os.path.getsize(abs_seg_path):
                            loaded_color_masks = []
                            for seg_idx, line in enumerate(loaded_seg_txt):
                                if seg_idx == 0:
                                    r, g, b = (255, 144, 30)
                                else:
                                    r, g, b = np.random.randint(100, 255), np.random.randint(100, 255), np.random.randint(100, 255)
                                loaded_class_num = line.strip()[0]
                                loaded_class_nums.append(loaded_class_num)
                                loaded_scaled_contour = line.strip()[2:].split()
                                loaded_scaled_contours.append(loaded_scaled_contour)

                                loaded_contour = [int(float(coor) * img_w) if c_idx % 2 == 0 else int(float(coor) * img_h) for c_idx, coor in enumerate(loaded_scaled_contour)]
                                loaded_contour = np.array(loaded_contour).reshape(-1, 1, 2)
                                loaded_contours.append(loaded_contour)
                    ...
                    with open(abs_bbox_path, 'r') as loaded_bbox_txt:
                        for bbox_idx, line in enumerate(loaded_bbox_txt):
                            if bbox_idx == 0:
                                r, g, b = (255, 0, 0)
                            else:
                                r, g, b = np.random.randint(100, 255), np.random.randint(100, 255), np.random.randint(100, 255)
                            loaded_class_num = line.strip()[0]
                            loaded_scaled_bbox = line.strip()[2:].split()
                            loaded_scaled_bboxes.append(loaded_scaled_bbox)
                            ...
                            loaded_bboxes.append([box_x, box_y, box_w, box_h])

                            cv2.rectangle(loaded_masked_image, (box_x, box_y), ((box_x + box_w), (box_y + box_h)), (r, g, b), 2)
                            cv2.putText(loaded_masked_image, loaded_class_num, (box_x + box_w // 2, box_y), cv2.FONT_HERSHEY_SIMPLEX, 1, (r, g, b), 3, cv2.LINE_AA)
    ...
    def imageClicked(self, event, label):
        ...
        if self.clear_all_check.isChecked():
            # [3] 불러온 Label 정보 초기화
            label.loaded_class_nums = []
            label.loaded_contours = []
            label.loaded_scaled_contours = []
            label.loaded_bboxes = []
            label.loaded_scaled_bboxes = []

            # [4] 새로 그린 Label 정보 초기화
            label.input_labels = []
            label.input_points = []
            label.scaled_points = []
            label.class_nums = []
        ...
        # - Label을 1개만 지우는 경우 or Label을 추가하는 경우
            ...
            if self.clear_only_one_check.isChecked():
                ...
                # 2]] 이후, 불러온 Label 제거
                elif label.loaded_class_nums:
                    label.loaded_class_nums.pop()
                    label.loaded_contours.pop()
                    label.loaded_scaled_contours.pop()
                    label.loaded_bboxes.pop()
                    label.loaded_scaled_bboxes.pop()
            ...
            if label.input_labels:
                # [6] SAM의 input image 세팅
                predictor.set_image(image)

                # - Mask 여러개 추출하는 경우
                if self.batch_check.isChecked():
                    # [7] 마스크 추출 - 여러개 Mask (Batch 단위)
                ...
                if len(masks):
                    ...
                    # pixmap_scaled = pixmap.scaled(300, 300, Qt.KeepAspectRatio)
                    # label.setPixmap(pixmap_scaled)
            # - Label 존재 하지 않는 경우 : Pass

        # [10] 마스크 영역 출력 by PyQt5
    ...
    def keyPressEvent(self, event):
        ...
        elif event.key() in [Qt.Key_0, Qt.Key_1, Qt.Key_2, Qt.Key_3, Qt.Key_4, Qt.Key_5, Qt.Key_6, Qt.Key_7, Qt.Key_8, Qt.Key_9]:
            input_class_num = event.key() - Qt.Key_0
            self.class_num_box.setText(str(input_class_num))
